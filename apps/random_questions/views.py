...
def generate_random_questions(request):
    ...
    exammodel_api = ExammodelApi()
    exam_sets = exammodel_api.find_all_exammodel({}, fields={'exam_code': 1})
    random_exam_codes = [exam_sets[random.randrange(len(exam_sets))]
                         ['exam_code'] for i in range(65)]
    print random_exam_codes.index(list(set(random_exam_codes))[0])
